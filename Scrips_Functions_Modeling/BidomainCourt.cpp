...
//Set fiber directions when provided file
void set_fiber_directions(libMesh::EquationSystems &es, const GetPot &data);
...
int main(int argc, char **argv)
{
    ...
    //ParallelMesh smallMesh(init.comm());
    ...
    //libMesh::EquationSystems es2(smallMesh);
    ...
    switch (time_integrator)
    {
        ...
        case TimeIntegrator::SEMI_IMPLICIT_HALF_STEP:
        {
            ...
            if(exportVMAT_time){
                ...
                LATSystem.add_variable("LAT",order, LAGRANGE);
            }
            ...
            parabolic_system.add_vector("ML2");
            ...
            elliptic_system.add_vector("ForcingVe");
        ...
        default:
        {
            ...
            system.add_vector("aux2"); // auxilliary vector for assembling the RHS
            system.add_vector("aux3"); // auxilliary vector for assembling the RHS
            ...
            system.add_vector("dVmdt_max_prev");
            system.add_vector("dVmdt_max_check");
            system.add_vector("dVmdt_max");
            system.add_vector("LATvec");
    ...
    switch (time_integrator)
    {
        ...
        case TimeIntegrator::SEMI_IMPLICIT_HALF_STEP:
        {
          exported_variables.insert("elliptic");
        ...
        default:
        {
          exported_variables.insert("bidomain");
    ...
    int numElectrodeValues = data("numElectrodeValues",340);

    bool surfacePointsEGMs = data("surfacePointsEGMs", false);
    ...
    vector<std::string> SurfaceNodes_coords;
    vector<double> SurfaceNodesXvec;
    vector<double> SurfaceNodesYvec;
    vector<double> SurfaceNodesZvec;

    vector<double> ActSurfaceNodesXvec;
    vector<double> ActSurfaceNodesYvec;
    vector<double> ActSurfaceNodesZvec;

    int surfacePointsCounter = 0;
    std::vector<std::vector<dof_id_type>> dof_indices_big_vector;

    if( surfacePointsEGMs ){
      std::string SurfaceNodes_coords_matrix = data("SurfaceNodes_coords_matrix", "fibrosisCoords.csv");
      std::string line, word;
      fstream file (SurfaceNodes_coords_matrix, ios::in);
      ...
      if(file.is_open())
      {
        ...
        {
          stringstream str(line);
          SurfaceNodes_coords.push_back(line);
          ...
          SurfaceNodesXvec.push_back( std::stod(CoordsInfoVec[0]) );
          SurfaceNodesYvec.push_back( std::stod(CoordsInfoVec[1]) );
          SurfaceNodesZvec.push_back( std::stod(CoordsInfoVec[2]) );
          surfacePointsCounter++;
        }

        libMesh::out << "Loading Surface geometry to calculate signals using the Vm's at " << surfacePointsCounter << " points, at: File: " << SurfaceNodes_coords_matrix << " found successfully..." << std::endl;
      }
      else{
        libMesh::out << "Could not open the Surface coodinates file..." << std::endl;
      }
    }

    std::vector < libMesh::Node > SurfaceNodesVector(surfacePointsCounter);
    ...
    es.parameters.set< double >("systemVectorContribution"+mesh.processor_id()) = 0.;
    ...
    //CHECK from before the x,y,z coordinates before doing anything else
    es.parameters.set<double>("specX") = 1000.;
    es.parameters.set<double>("specY") = 1000.;
    es.parameters.set<double>("specZ") = 1000.;
     ...
     ofstream foutputV; 
     ifstream finputV;
     finputV.open (nameOfVFileExt);
     foutputV.open (nameOfVFileExt,ios::app); 

     ofstream foutputIm; 
     ifstream finputIm;
     finputIm.open (nameOfImFileExt);
     foutputIm.open (nameOfImFileExt,ios::app); 

     ofstream foutputx; 
     ifstream finputx;
     finputx.open (nameOfxFileExt);
     foutputx.open (nameOfxFileExt,ios::app); 

     ofstream foutputy; 
     ifstream finputy;
     finputy.open (nameOfyFileExt);
     foutputy.open (nameOfyFileExt,ios::app); 

     ofstream foutputz; 
     ifstream finputz;
     finputz.open (nameOfzFileExt);
     foutputz.open (nameOfzFileExt,ios::app); 
    ...
    for (; datatime.time < datatime.end_time; )
    {
        ...
        switch (time_integrator)
        {
            ...
            case TimeIntegrator::SEMI_IMPLICIT_HALF_STEP:
            {
                ...
                *parabolic_system.old_local_solution = *parabolic_system.solution;
                ...
                //libMesh::out << "TRACKING BUG 1" << std::endl;
            ...
            case TimeIntegrator::SBDF3:
            {
                if(datatime.timestep == 1)
                {
                    ...
                    es.get_system("bidomain").get_vector("R_dist").zero();                   
                }
                ...
                *system.old_local_solution = *system.solution;
                ...
                SolverRecovery (es, data, datatime);
                if(datatime.timestep == 1){
                  assemble_rhs(es, datatime, TimeIntegrator::SBDF1, order, data);
                }
                ...
                system.get_vector("Vnprev") = *system.old_local_solution;
        ...
        //if(exportVe_mono && 0. == ( (lround((datatime.timestep*datatime.dt*100))) % (lround(exportInterval*100)) ) ){
            ...
            if( time_integrator == TimeIntegrator::SBDF1 || time_integrator == TimeIntegrator::SBDF2 || time_integrator == TimeIntegrator::SBDF3 ){
              //const unsigned int dim = mesh.mesh_dimension();
              const DofMap & dof_mapP = es.get_system("bidomain").get_dof_map();
              auto femSoluV = es.get_system("bidomain").variable_number("V");
              libMesh::TransientLinearImplicitSystem &system = es.get_system < libMesh::TransientLinearImplicitSystem > ("bidomain");
        ...
        //Basically, in every processor we check the box created by the threshDist +- the point of interest.
        //Then all the values of Ve are moved to processor 0, and based on the minimum distance, that Ve is exported.
        //threshDist has to be small enough to get only one value but large enough that the point is within the box.
        if(exportVe_time && 0 == datatime.timestep % extraMeshInt){
            libMesh::out << "Exporting Ve and V at t = " << datatime.time << std::endl;
            double Ve, x, y, z, diffX, diffY, diffZ;
            double threshDist = data("threshDist", 0.1);
            std::vector<double> locVe, locDist;
            int nelz = data("nelz",0);
  
            std::string xE = data("exportVe_timeX", "0.");
            std::string yE = data("exportVe_timeY", "0.");
            std::string zE = data("exportVe_timeZ", "0.");
            ...
            {
              const unsigned int dim = mesh.mesh_dimension();
              const DofMap & dof_map2 = es.get_system("bidomain").get_dof_map();
              const DofMap & dof_mapP = es.get_system("Recovery").get_dof_map();
              //std::vector<dof_id_type> dof_indices;
              auto femSolu2 = es.get_system("bidomain").variable_number("Ve");
              auto femSoluV = es.get_system("bidomain").variable_number("V");
              libMesh::TransientLinearImplicitSystem &system = es.get_system < libMesh::TransientLinearImplicitSystem > ("bidomain");
              libMesh::TransientExplicitSystem &systemP = es.get_system < libMesh::TransientExplicitSystem > ("Recovery");

              if( numElectrodeValues == 1 ){
                //if (mesh.processor_id() == 0) {
                  Point point_D(xE1, yE1, zE1);
                  VeBido = system.point_value(femSolu2, point_D);
                  locDistanceMin = .005;//sqrt( pow((x - xE1),2) + pow((y - yE1),2) + pow((z - zE1),2) );
                //}
                ...
                /*
                  for (auto node : mesh.local_node_ptr_range()){
                    ...
                    //if( mesh.processor_id() == node -> processor_id() ){
                        dof_mapP.dof_indices(node, dof_indicesP);
                        dof_map2.dof_indices(node, dof_indices);
                        x = (*node)(0);
                        y = (*node)(1);
                        ...
                        if(dof_indices.size() > 0){
                          if( std::abs(x - xE1) < dXmesh && std::abs(y - yE1) < dXmesh && std::abs(z - zE1) < dXmesh ){
                              //VeBido = (*system.solution)  (dof_indices[femSolu2]);
                              Point point_D(x, y, z);
                              VeBido = system.point_value(femSolu2, point_D);
                              locDistanceMin = sqrt( pow((x - xE1),2) + pow((y - yE1),2) + pow((z - zE1),2) );
                              break;
                              //if(dof_indices.size() == dof_indicesP.size()) {
                                //V_Cath_vector.push_back((*systemP.solution)  (dof_indicesP[femSoluV]));
                                //V_Cath_vector[counterVe] = (*systemP.solution)  (dof_indicesP[femSoluV]);
                              //}
                              //processorOfIntBi = true;
                              //varInterestSet = femSolu2;
                              //dof_indicesSet = dof_indices;
                ...
                //if(datatime.timestep > 1 && processorOfIntBi){
                  //VeBido = (*system.solution)  (dof_indicesSet[varInterestSet]);
                  //locDistanceMin = .00001;
                //}
              }
              else{
              ...
              const unsigned int dim = mesh.mesh_dimension();
              const DofMap & dof_map2 = es.get_system("elliptic").get_dof_map();
              const DofMap & dof_mapP = es.get_system("parabolic").get_dof_map();
              //std::vector<dof_id_type> dof_indices;
              auto femSolu2 = es.get_system("elliptic").variable_number("Ve");
              auto femSoluV = es.get_system("parabolic").variable_number("V");
              libMesh::LinearImplicitSystem &system = es.get_system < libMesh::LinearImplicitSystem > ("elliptic");
              libMesh::TransientLinearImplicitSystem &systemP = es.get_system < libMesh::TransientLinearImplicitSystem > ("parabolic");
              ...
              if( numElectrodeValues == 1 ){
                ...
                /*
                  for (auto node : mesh.local_node_ptr_range()){
                        ...
                        y = (*node)(1);
                        z = (*node)(2);
                        if( std::abs(x - xE1) < dXmesh*1. && std::abs(y - yE1) < dXmesh*1. && std::abs(z - zE1) < dXmesh*1. ){
                            VeBido = (*system.solution)  (dof_indices[femSolu2]);
                            locDistanceMin = sqrt( pow((x - xE1),2) + pow((y - yE1),2) + pow((z - zE1),2) );
                            //if(dof_indices.size() == dof_indicesP.size()) {
                              //V_Cath_vector.push_back((*systemP.solution)  (dof_indicesP[femSoluV]));
            ...
            if( numElectrodeValues == 1 ){
              ...
              double *vecPhiBi = NULL;
              double *vecDista = NULL;

              if (mesh.processor_id() == 0) {
                int numprocessors = mesh.n_processors();
                vecPhiBi = (double *) malloc(numprocessors*sizeof(double)) ;
                vecDista = (double *) malloc(numprocessors*sizeof(double)) ;
              }

              MPI_Gather(&VeBido, 1, MPI_DOUBLE, vecPhiBi, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
              MPI_Gather(&locDistanceMin, 1, MPI_DOUBLE, vecDista, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);


              if(mesh.processor_id() == 0){

                  double localvaluePhiEB = 0.;
                  double distanceMesh = 100.;

                  double totalDistSum = 0.;

                  for(int i = 0; i < mesh.n_processors() ; i++){
                    if( vecDista[i] < distanceMesh/2.){
                      localvaluePhiEB = vecPhiBi[i];
                      //distanceMesh = vecDista[i];
                      //libMesh::out << "at point with distance " << distanceMesh <<std::endl;
                    }
                    totalDistSum += vecDista[i];
                    //libMesh::out << "Ve value is " << vecPhiBi[i] << " and distance is " << vecDista[i] << std::endl;
                  }
                  //mean of distances is the same, then never got a point
                  //if( totalDistSum / mesh.n_processors() == locDistanceMin ){
                    //libMesh::out << "Choose a different dX for the 'mesh' size since no point was found for this electrode coordinates..." << std::endl;
                  //} 
                  //else{
                    Ve_Cath_vector[counterVe] = VeBido;//localvaluePhiEB;//std::accumulate(processorContribution.begin(), processorContribution.end(), 0.0);
                  //}
                counterVe++;
              }
            }
              ...
              if(mesh.processor_id() == 0){
                ...
                for(int i = 0; i < Ve_Cath_vector_curr.size(); i++){
                    if( i != Ve_Cath_vector_curr.size()-1 ){
                        currentTotalPhiE += to_string( Ve_Cath_vector_curr[i] ) + ",";
                    }
                    else{
                        currentTotalPhiE += to_string( Ve_Cath_vector_curr[i] );
                    }
                }
                Ve_Cath_vector_string[counterVe] = currentTotalPhiE;//std::accumulate(processorContribution.begin(), processorContribution.end(), 0.0);
                counterVe++;
              }
            }
        ...
        //if(exportVe_mono && 0. == ( (lround((datatime.timestep*datatime.dt*100))) % (lround(exportInterval*100)) ) ){
            ...
            double avgNodeDist = data("avgNodeDist", 0.05);
            ...
            */
            ...
            if( time_integrator == TimeIntegrator::SBDF1 || time_integrator == TimeIntegrator::SBDF2 || time_integrator == TimeIntegrator::SBDF3 ){
              ...
              int intervalOfElectrodesChecked = data("intervalOfElectrodesChecked", 1);
              //phi_E = 0.;
              ...
              if(surfacePointsEGMs && numExportPoints < 1){
                  for (auto node : mesh.local_node_ptr_range())
                  {
                        ...
                        if (parabolic_ndofs > 0)
                        {
                                ...
                                for(int checkCoords = 0; checkCoords < SurfaceNodesXvec.size(); checkCoords++){
                                  double distX = std::abs( x - SurfaceNodesXvec[checkCoords] );
                                  double distY = std::abs( y - SurfaceNodesYvec[checkCoords] );
                                  double distZ = std::abs( z - SurfaceNodesZvec[checkCoords] );
                                  if( distX < avgNodeDist && distY < avgNodeDist && distZ < avgNodeDist ){
                                    
                                    dof_indices_big_vector.push_back(dof_indices);
                                    ActSurfaceNodesXvec.push_back(x);
                                    ActSurfaceNodesYvec.push_back(y);
                                    ActSurfaceNodesZvec.push_back(z);
              ...
              if( numElectrodeValues == 1  ){
                if(surfacePointsEGMs){
                    for (int i = 0; i < ActSurfaceNodesXvec.size(); i++)
                    {

                          double x = ActSurfaceNodesXvec[i];
                          double y = ActSurfaceNodesYvec[i];
                          double z = ActSurfaceNodesZvec[i];
                          double BImloc = (system.get_vector("aux1"))  (dof_indices_big_vector[i][femSoluV]);
                  ...
                  for (auto node : mesh.local_node_ptr_range())
                  {
                        ...
                        if (parabolic_ndofs > 0)
                        {
                                ...
                                double r_distLoc = ( sqrt( (xE1 - x)*(xE1 - x) + (yE1 - y)*(yE1 - y) + (zE1 - z)*(zE1 - z) ) );
                                double BImloc = (system.get_vector("aux1"))  (dof_indices[femSoluV]);//( 1./( 4.*pi*sigma_b*chi ) )*( Cm*(Vcurloc - Vprevloc)/datatime.dt - Inloc );//(system.get_vector("aux1"))  (dof_indicesP[femSoluV]);
                                phi_E += (chi/(4.*pi*sigma_b)) * (BImloc / r_distLoc);  
                        }
                  }
                }
                  ...
                  numExportPoints++;
                  for(int i = 0; i < numElectrodeValues; i = i + intervalOfElectrodesChecked){
                      xE1 = electrodeXvec[i];
                      yE1 = electrodeYvec[i];
                      zE1 = electrodeZvec[i];

                      if(surfacePointsEGMs){
                          for (int j = 0; j < ActSurfaceNodesXvec.size(); j++)
                          {

                                double x = ActSurfaceNodesXvec[j];
                                double y = ActSurfaceNodesYvec[j];
                                double z = ActSurfaceNodesZvec[j];
                                double BImloc = (system.get_vector("aux1"))  (dof_indices_big_vector[j][femSoluV]);
                                double r_distLoc = ( sqrt( (xE1 - x)*(xE1 - x) + (yE1 - y)*(yE1 - y) + (zE1 - z)*(zE1 - z) ) );
                                phi_E += (chi/(4.*pi*sigma_b)) * (BImloc / r_distLoc);  
                          }
                      }
                      ...
                      if(mesh.processor_id() == 0){
                        ...
                        Ve_Cath_Mono_vector_curr[i] = localvaluePhiE;
                      }
                      phi_E = 0.;
                  }
              ...
              if(surfacePointsEGMs && numExportPoints < 1){
                  for (auto node : mesh.local_node_ptr_range())
                  {
                        ...
                        if (parabolic_ndofs == elliptic_ndofs)
                        {
                                double x = (*node)(0);
                                double y = (*node)(1);
                                double z = (*node)(2);
                                ...
                                for(int checkCoords = 0; checkCoords < SurfaceNodesXvec.size(); checkCoords++){
                                  double distX = std::abs( round_up(x,8) - SurfaceNodesXvec[checkCoords] );
                                  double distY = std::abs( round_up(y,8) - SurfaceNodesYvec[checkCoords] );
                                  double distZ = std::abs( round_up(z,8) - SurfaceNodesZvec[checkCoords] );
                                  if( distX < avgNodeDist && distY < avgNodeDist && distZ < avgNodeDist ){
                                    
                                    dof_indices_big_vector.push_back(dof_indicesP);
                                    ActSurfaceNodesXvec.push_back(x);
                                    ActSurfaceNodesYvec.push_back(y);
                                    ActSurfaceNodesZvec.push_back(z);
                ...
                if(surfacePointsEGMs){
                  ...
                  for (auto node : mesh.local_node_ptr_range())
                  {
                      ...
                      if ( elliptic_ndofs == parabolic_ndofs )
                      {
                              ...
                              //phi_E += ( chi/( 4.*pi*sigma_eTot ) ) * (BImloc / r_distLoc);
                              phi_E += (chi/(4.*pi*sigma_b)) * (BImloc / r_distLoc);
                              //system.get_vector("R_dist").set( dof_indicesP[femSoluV], (1. / r_distLoc) ); 
                      }
                  }
                }
                //}
            ...
            if( numElectrodeValues == 1 ){
              ...
              MPI_Gather(&phi_E, 1, MPI_DOUBLE, vecPhiEval, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
              //double testProcessors = mesh.processor_id()*3.0;
              //double *vecTestProcessors = NULL;
              //if (mesh.processor_id() == 0) {
                //int numprocessors = mesh.n_processors();
                //vecTestProcessors = (double *) malloc(numprocessors*sizeof(double)) ;
              //}
              //MPI_Gather(&testProcessors, 1, MPI_DOUBLE, vecTestProcessors, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
              if(mesh.processor_id() == 0){
                //if(mesh.processor_id() == 0){
                  double localvaluePhiE = 0.;
                  for(int i = 0; i < mesh.n_processors() ; i++){
                  ...
                  //double localvaluePhiE = std::accumulate(vecPhiEval.begin(), vecPhiEval.end(), 0.0);
                  //libMesh::out << "The value is: " << localvaluePhiE << std::endl;
                //}
                //es.parameters.set< double >("systemVectorContribution"+mesh.processor_id()) = calculate_PhiE(es, datatime, time_integrator, order, data);
                Ve_Cath_Mono_vector[counterVeMono] = localvaluePhiE;// / mesh.n_processors();//std::accumulate(processorContribution.begin(), processorContribution.end(), 0.0);
                //es.parameters.set< double >("phiEvalue") += Ve_Cath_Mono_vector[counterVeMono];
                //for(int j = 0; j < mesh.n_processors(); j++){
                  //Ve_Cath_Mono_vector[counterVeMono] += std::accumulate(processorContribution.begin(), processorContribution.end(), 0.0);
                  //Ve_Cath_Mono_vector[counterVeMono] += es.parameters.get< double >("systemVectorContribution"+mesh.processor_id()) ;
                  //libMesh::out << es.parameters.get< double >("systemVectorContribution"+mesh.processor_id()) << std::endl;
                  //libMesh::out << "Total nodes contributing: " << numNonzero << std::endl;
                //}
                counterVeMono++;
                //double locCheckP = 0.;
                //for(int i = 0; i < mesh.n_processors() ; i++){
                  //locCheckP += vecTestProcessors[i];
                //}
                //libMesh::out << "Summation is: " << locCheckP << " and Average is: " << locCheckP/mesh.n_processors() << std::endl;
              }
            }
            else{
    ...
    */
    ...
    if(exportVe_time){
      ...
      if(mesh.processor_id() == 0){
        if( numElectrodeValues == 1 ){
          ...
          std::string nameOfOutputFile2, nameOfOutputFileV1;
          std::string nameOfTimeFile;
          nameOfOutputFile2 = "Ve_Catheter_"+integrator+"_"+exportVe_Name+"_"+std::to_string(counterVe)+"points.csv";
          //nameOfTimeFile = "t_vector_"+integrator+"_"+exportVe_Name+"_"+std::to_string(counterVe)+"points.txt";
          //outputFile2.open (nameOfOutputFile2);
          //outputFile2 << (Ve_Cath_vector)<< std::endl;
          //outputFile2.close();
          ...
          std::copy(Ve_Cath_vector.begin(), Ve_Cath_vector.end(), output_iterator);
          ...
          //ofstream output_file3(nameOfTimeFile);
          //std::ostream_iterator<double> output_iterator2(output_file3, "\n");
          //std::copy(t_vector.begin(), t_vector.end(), output_iterator2);

          //libMesh::out << "Printing the vector:" << std::endl;
          //for(int i=0; i<Ve_Cath_vector.size(); i++){
            //libMesh::out << Ve_Cath_vector[i] <<  ",";
          //}
        }
        else{
          //ofstream outputFile2;
          ...
          std::copy(Ve_Cath_vector_string.begin(), Ve_Cath_vector_string.end(), output_iterator);

          //nameOfOutputFileV1 = "V_Catheter_"+integrator+"_"+exportVe_Name+"_"+std::to_string(counterVe)+"points.txt";
          //ofstream output_fileV1(nameOfOutputFileV1);
          //std::ostream_iterator<double> output_iteratorV1(output_fileV1, "\n");
          //std::copy(V_Cath_vector.begin(), V_Cath_vector.end(), output_iteratorV1);
    ...
    if(exportVe_mono){
      ...
      if(mesh.processor_id() == 0){
        if( numElectrodeValues == 1 ){
          ...
          std::string nel2 = data("nelx", "50");
          std::string exportVe_Name = data("exportVe_Name","ExtracellularNegative_NoFibrosis");
          std::string nameOfOutputFile2, nameOfOutputFileCoords;
          std::string nameOfTimeFile;
          nameOfOutputFile2 = "Ve_Catheter_MonoAp_"+integrator+"_"+exportVe_Name+"_"+std::to_string(counterVeMono)+"points.csv";
          nameOfOutputFileCoords = "Ve_Catheter_Coords_MonoAp_"+integrator+"_"+exportVe_Name+"_"+std::to_string(counterVeMono)+"points.csv";
    ...
    if(convergence_test)
    {
        ...
        std::string nameOfOutputFile = "outPut_for_"+integrator+"_"+nel2+"elems_"+eltype2+"type.csv";
...
void read_mesh(const GetPot &data, libMesh::ParallelMesh &mesh, libMesh::SerialMesh &meshRef)
{
    ...
    //std::string mesh_name = data("mesh_filename", "NONE");
    ...
    if (mesh_filename.compare("NONE") != 0)
    {
        ...
        bool idealLAPW = data("idealLAPW", false);
        int orientationAxis = data("orientationAxis", 2);// 0 for x, 1 for y, 2 for z
        double smallRadiusEllipse = data("smallRadiusEllipse", 1.0);
        double longRadiusEllipse = data("longRadiusEllipse", 1.0);


        if(mesh_refinement_factor_tissue > 0){
          MeshRefinement meshR(mesh);
          //MeshRefinement meshR2(meshRef);
          libMesh::out << "Refining mesh " << mesh_refinement_factor_tissue << " times..." << std::endl;
          ...
          libMesh::out << "Mesh refinement done..." << std::endl;
        }
        ...
        int subDomainNum = data("subDomainNum",1);
        int numFibrosisPoints = 0, numTissuePoints = 0;
        //if 1: then it's only tissue
        //if 2: then it's tissue and batg
        //if 3: then it's tissue, bath, and fatty layer (or something else on the other side of the tissue)
        if(subDomainNum == 1){
        ...
        else if(subDomainNum == 2){
          std::string mesh_tissueRef = data("mesh_tissueRef", "NONE");
          meshRef.read(mesh_tissueRef);
          std::string check_Presence;

          if(idealLAPW){
                std::cout << "Ideal Realistic Geometry checking with " << orientationAxis << " axis orientation and: " << smallRadiusEllipse << " and " << longRadiusEllipse << " for the small and long radius, respectively" << std::endl;
                for(auto el: mesh.element_ptr_range()){
                  ...
                  //auto localNodes = el -> get_nodes();
                  //auto numNodes = el -> n_nodes();
                  //std::vector<int> tissueBool = {2, 2, 2, 2};
                  //int max = 0, indToUse = 0, mostvalue = 2;
                  ...
                  //for(int locnode = 0; locnode < numNodes; locnode++){
                    ...
                    else if(orientationAxis == 1){
                      //Y - axis oriented

                    }
                    else if(orientationAxis == 2){
                      //Z - axis oriented
                      if( ( ((x*x)/(smallRadiusEllipse*smallRadiusEllipse)) + ((z*z)/(longRadiusEllipse*longRadiusEllipse)) ) >= 1.0 ){
                        //tissueBool[locnode] = 1;
                        el->subdomain_id() = static_cast<libMesh::subdomain_id_type>(Subdomain::TISSUE);
                      }
                      else{
                        //tissueBool[locnode] = 0;
                        el->subdomain_id() = static_cast<libMesh::subdomain_id_type>(Subdomain::BATH);
                        //libMesh::out<< "in bath subdomain yeaaah" << std::endl;
                      }
                    }
                  //}

                  /*
                  double avgTiss=0., numNonTwo=0.;
                  for (int i = 0;i < tissueBool.size(); i++){
                    if(tissueBool[i] != 2){
                      avgTiss += (double) tissueBool[i];
                      numNonTwo = numNonTwo + 1.;
                    }
                  }
                  if( avgTiss/numNonTwo >= thresholdPerElement){
                    el->subdomain_id() = static_cast<libMesh::subdomain_id_type>(Subdomain::TISSUE);
                  }
                  else{
                    el->subdomain_id() = static_cast<libMesh::subdomain_id_type>(Subdomain::BATH);
            ...
            for(auto elRef: meshRef.element_ptr_range()){
                ...
                for(auto el: mesh.element_ptr_range()){
                  if(el -> contains_point(c) ){
                    //IF HERE, THEN FOR SURE WE ARE IN TISSUE OR FIBROSIS DOMAIN
                    el->subdomain_id() = static_cast<libMesh::subdomain_id_type>(Subdomain::TISSUE);
                  } 
                  else{
                    if( el->subdomain_id() == static_cast<libMesh::subdomain_id_type>(Subdomain::TISSUE) ){
                      //DON'T DO ANYTHING BECAUSE THE ELEMENT IS ALREADY TISSUE OR FIBROSIS
                    }
                    else{
                      el->subdomain_id() = static_cast<libMesh::subdomain_id_type>(Subdomain::BATH);
                    }
          ...
          if(fibrosisBool && fibrosisMethod.compare(0,11,"Percolation") == 0){
              for(auto el: mesh.element_ptr_range()){
                ...
                if( el->subdomain_id() == static_cast<libMesh::subdomain_id_type>(Subdomain::TISSUE) || el->subdomain_id() == static_cast<libMesh::subdomain_id_type>(Subdomain::FIBROSIS) ){
                    //check for fibrosis
                    auto localNodes = el -> get_nodes();
                    auto numNodes = el -> n_nodes();
                    std::vector<int> fibBool = {2, 2, 2, 2};
            ...
            libMesh::out << "Done adding fibrotic elements specification via subdomain splitting..." << std::endl;
          }
          

        }
        else if(subDomainNum == 3){
          std::string check_Presence;
        }
...
void assemble_matrices(libMesh::EquationSystems &es, const TimeData &timedata, TimeIntegrator time_integrator, libMesh::Order p_order, const GetPot &data)
{
  ...
  if(fibrosisBool && fibrosisMethod.compare(0,14,"Conductivities") == 0){
    ...
    */
    ...
    double groundNodeX = data("groundNodeX", .5);
    double groundNodeY = data("groundNodeY", .5);
    double groundNodeZ = data("groundNodeZ", .5);

    double threshDist = data("threshDist", .1);
    int subDomainNum = data("subDomainNum", 1);

    std::string groundNodeXs = data("groundNodeXs", "");
    std::string groundNodeYs = data("groundNodeYs", "");
    std::string groundNodeZs = data("groundNodeZs", "");

    vector<double> groundNodeXvec2;
    vector<double> groundNodeYvec2;
    vector<double> groundNodeZvec2;

    int groundingWay = data("groundingWay", 4);

    if(subDomainNum > 1 && groundingWay == 5){
      std::string groundedPointsCoords = data("groundedPointsCoords", "NONE.csv");

      std::string line, word;
      fstream file (groundedPointsCoords, ios::in);
      ...
      if(file.is_open())
      {
        while(getline(file, line))
        {
          ...
          groundNodeXvec2.push_back( std::stod(CoordsInfoVec[0]) );
          groundNodeYvec2.push_back( std::stod(CoordsInfoVec[1]) );
          groundNodeZvec2.push_back( std::stod(CoordsInfoVec[2]) );


        }
        libMesh::out << "Importing the coordinate points to ground " << groundNodeXvec2.size() << " specific nodes: File: " << groundedPointsCoords << " found successfully..." << std::endl;
      }
      else{
        libMesh::out << "Could not open the file..." << std::endl;
      }
    }
    //X 1, Y 2, Z 3, all 4, list 5

    vector<double> groundNodeXvec;
    vector<double> groundNodeYvec;
    vector<double> groundNodeZvec;

    stringstream groundedNodesX(groundNodeXs);
    stringstream groundedNodesY(groundNodeYs);
    stringstream groundedNodesZ(groundNodeZs);

    if(groundNodeXs.length() > 0){

      while (groundedNodesX.good()) {
          string substr;
          getline(groundedNodesX, substr, ',');
          groundNodeXvec.push_back( std::stod( substr ) );
      }
      while (groundedNodesY.good()) {
          string substr;
          getline(groundedNodesY, substr, ',');
          groundNodeYvec.push_back( std::stod( substr ) );
      }
      while (groundedNodesZ.good()) {
          string substr;
          getline(groundedNodesZ, substr, ',');
          groundNodeZvec.push_back( std::stod( substr ) );
      }
      libMesh::out << "Using " << groundNodeXvec.size() << " prescribed coordinate points to ground nodes..." << std::endl;

    }
    else{
      libMesh::out << "No nodes to ground prescribed..." << std::endl; 
    }
    ...
    bool differentFibersFlip = data("differentFibersFlip", false);
    bool simulatedLAPWFibers = data("simulatedLAPWFibers", false);
    ...
    switch (time_integrator)
    {
        ...
        case TimeIntegrator::SEMI_IMPLICIT_HALF_STEP:
        {
            ...
            for (auto elem : mesh.active_local_element_ptr_range())
            {
                            ...
                            */
                            ...
                            for (unsigned int j = 0; j != parabolic_ndofs; j++)
                            {
                              ...
                              if(differentFibers){
                                ...
                                double fxloc, fyloc, sxloc, syloc;

                                //SPECIFIC PATTERN THAT TRIES TO RESEMBLE WHAT OBSERVED KIND OF IN REALISTIC GEOMETRIES
                                if(simulatedLAPWFibers){
                                  if( x < ( minx+((maxx-minx)*.1) ) ){
                                    fxloc = sx;
                                    fyloc = sy;
                                    ...
                                    if(differentFibersFlip){
                                      if( y > ( miny+((maxy-miny)*.25) ) ){
                                        fxloc = cos( ( differentFibersAngle * acos(-1.) )/180. );
                                        fyloc = sin( ( differentFibersAngle * acos(-1.) )/180. );

                                        sxloc = -cos( ( differentFibersAngle * acos(-1.) )/180. );
                                        syloc = sin( ( differentFibersAngle * acos(-1.) )/180. );
                                      }
                                      else{
                                        sxloc = cos( ( differentFibersAngle * acos(-1.) )/180. );
                                        syloc = sin( ( differentFibersAngle * acos(-1.) )/180. );
                                ...
                                TensorValue<Number> fofloc(fxloc * fxloc, fxloc * fyloc, fxloc * fz, fyloc * fxloc, fyloc * fyloc, fyloc * fz, fz * fxloc, fz * fyloc, fz * fz);
                                TensorValue<Number> sosloc(sxloc * sxloc, sxloc * syloc, sxloc * sz, syloc * sxloc, syloc * syloc, syloc * sz, sz * sxloc, sz * syloc, sz * sz);


                                TensorValue<Number> sigma_iloc = ( sigma_f_i * fofloc + sigma_s_i * sosloc + sigma_n_i * non ) /chi;
                                TensorValue<Number> sigma_eloc = ( sigma_f_e * fofloc + sigma_s_e * sosloc + sigma_n_e * non ) /chi;
    




                                  K(i, j) += JxW[qp] * dphi[i][qp] * (((sigma_iloc + sigma_eloc)/factorDiv) * dphi[j][qp]);
                                  Ki(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_iloc/factorDiv) * dphi[j][qp]);
                                  Kmono(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_M/factorDiv) * dphi[j][qp]);

                                  auto xE1 = exportVe_timeX;
                                  auto yE1 = exportVe_timeY;
                                  auto zE1 = exportVe_timeZ;
                                  //r_dist = phi[i][qp]*sqrt( (xT - xE1)*(xT - xE1) + (yT - yE1)*(yT - yE1) + (zT - zE1)*(zT - zE1) );
                                  double r_dist = sqrt( (xT - xE1)*(xT - xE1) + (yT - yE1)*(yT - yE1) + (zT - zE1)*(zT - zE1) );

                                  Ke(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_eloc/factorDiv) * dphi[j][qp]);
                                  S(i, j)  += JxW[qp] * dphi[i][qp] * ((sigma_iloc/factorDiv) * dphi[j][qp]);

                                
                              }
                              else{
                                K(i, j) += JxW[qp] * dphi[i][qp] * (((sigma_I_fibersV[indToUse] + sigma_E_fibersV[indToUse])/factorDiv) * dphi[j][qp]);
                                ...
                                //r_dist = phi[i][qp]*sqrt( (xT - xE1)*(xT - xE1) + (yT - yE1)*(yT - yE1) + (zT - zE1)*(zT - zE1) );
                                double r_dist = sqrt( (xT - xE1)*(xT - xE1) + (yT - yE1)*(yT - yE1) + (zT - zE1)*(zT - zE1) );
                                //Kmono(i, j) += JxW[qp] * (( -1. )/( 4. * pi * sigma_b_ie * r_dist )) * dphi[i][qp] * ((sigma_M) * dphi[j][qp]);


                                Ke(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_E_fibersV[indToUse]/factorDiv) * dphi[j][qp]);
                                S(i, j)  += JxW[qp] * dphi[i][qp] * ((sigma_I_fibersV[indToUse]/factorDiv) * dphi[j][qp]);
                              }
                    ...
                    for (unsigned int i = 0; i != elliptic_ndofs; i++)
                    {
                      for (unsigned int j = 0; j != elliptic_ndofs; j++)
                      {
                        ...
                        if(differentFibers){
                            ...
                            // stiffness matrix
                            K(i, j) += JxW[qp] * dphi[i][qp] * (((sigma_iloc + sigma_eloc)) * dphi[j][qp]);
                          
                        }
                        else{
                          // stiffness matrix
                          K(i, j) += JxW[qp] * dphi[i][qp] * (((sigma_I_fibersV[indToUse] + sigma_E_fibersV[indToUse])) * dphi[j][qp]);
                        }
                            ...
                            if (elem->neighbor_ptr(side) != nullptr)
                            {
                                ...
                                if(subdomain_id != neighbor_subdomain_id)
                                {
                                    ...
                                    for (unsigned int qp = 0; qp < qface.n_points(); qp++)
                                    {
                                        for (unsigned int i = 0; i != parabolic_ndofs; i++)
                                        {
                                            const double xT = q_point_face[qp](0);
                                            const double yT = q_point_face[qp](1);
                                            const double zT = q_point_face[qp](2);
                                            ...
                                            {
                                                double r_dist = sqrt( (xT - xE1)*(xT - xE1) + (yT - yE1)*(yT - yE1) + (zT - zE1)*(zT - zE1) );
                                                Kmono(i, j) -= JxW_face[qp] * ( ( sigma_M * dphi_face[j][qp] ) * normal[qp] ) * phi_face[i][qp]; //+= JxW_face[qp] * penalty * phi_face[j][qp] * phi_face[i][qp];
                ...
                if (subdomain_id == static_cast<libMesh::subdomain_id_type>(Subdomain::BATH) || subdomain_id == static_cast<libMesh::subdomain_id_type>(Subdomain::TORSO))
                {
                    ...
                    if( subDomainNum > 1){//&& mesh_filename.compare(0,14,"NONE") != 0
                        double x1 = c(0);
                        double y1 = c(1);
                        double z1 = c(2);
                          ...
                          switch(groundingWay){
                            ...
                            //ALL
                            case 4:{
                              for( int grp = 0 ; grp < groundNodeXvec.size() ; grp++ ){
                                if ( std::abs(groundNodeXvec[grp] - x1) < threshDist && std::abs(groundNodeYvec[grp] - y1) < threshDist && std::abs(groundNodeZvec[grp] - z1) < threshDist )
                                {
                                    for (unsigned int qp = 0; qp < qrule.n_points(); qp++)
                                    {
                                        for (unsigned int i = 0; i != elliptic_ndofs; i++)
                                        {
                                            for (unsigned int j = 0; j != elliptic_ndofs; j++)
                                            {
                                                K(i, j) += JxW[qp] * penalty * phi[j][qp] * phi[i][qp];
                                            }
                                        }
                                    }
                                }
                            ...
                            case 5:{
                              for( int grp = 0 ; grp < groundNodeXvec2.size() ; grp++ ){
                                if ( std::abs(groundNodeXvec2[grp] - x1) < threshDist && std::abs(groundNodeYvec2[grp] - y1) < threshDist && std::abs(groundNodeZvec2[grp] - z1) < threshDist )
                                {
                                  for (auto side : elem->side_index_range())
                                  {
                                      if (elem->neighbor_ptr(side) == nullptr)
        ...
        default:
        {
            ...
            system.get_matrix("Ki").zero();
            system.get_matrix("Kmono").zero();
            ...
            {
                auto c = elem -> centroid();
                //libMesh::out << "TRACKING BUG 2" << std::endl;
                ...
                if(prescribedFibers){
                  ...
                  //auto actualNumNodes = (*localNodes) -> size();
                  //libMesh::out << "actual number of local nodes is " << actualNumNodes << std::endl;
                  ...
                  //libMesh::out << "size is: " << sizeof(localNodes) << " and type is: " << typeid(localNodes).name() << std::endl;
                  //libMesh::out << "Checking how many per element " << numNodes << std::endl;
                  //for ( auto locnode = *localNodes.begin(); locnode != *localNodes.end(); ++locnode ){
                  for(int locnode = 0; locnode < numNodes; locnode++){

                    if( localNodes[locnode] -> processor_id() == mesh.processor_id() ){

                      recovery_dof_map.dof_indices(localNodes[locnode], node_dof_indices);
                      if(node_dof_indices.size() > 0){
                        double x = (*localNodes[locnode])(0);
                        double y = (*localNodes[locnode])(1);
                        double z = (*localNodes[locnode])(2);

                        check_Presence = to_string_with_precision( round_up(x,8), 4 )+","+to_string_with_precision( round_up(y,8), 4 )+","+to_string_with_precision( round_up(z,8), 4 );
                        //libMesh::out << "Local point:" << std::endl;
                        //libMesh::out << check_Presence << std::endl;
                        //libMesh::out << "Point in CSV:" << std::endl;
                        //libMesh::out << CoordsPoints[0] << std::endl;
                        ...
                        //}
                          ...
                          libMesh::out << "in node fx is: " << fx << std::endl;
                ...
                */
                ...
                //libMesh::out << "TRACKING BUG 3" << std::endl;
                auto subdomain_id = elem->subdomain_id();
                // if we are in the bath
                if (subdomain_id == static_cast<short unsigned int>(Subdomain::BATH) || subdomain_id == static_cast<short unsigned int>(Subdomain::TORSO))
                {
                ...
                else if(subdomain_id == static_cast<short unsigned int>(Subdomain::TISSUE))
                {
                    for (unsigned int qp = 0; qp < qrule.n_points(); qp++)
                    {
                        ...
                        if( prescribedFibers ){
                            libMesh::LinearImplicitSystem & fiberSoln = es.add_system <libMesh::LinearImplicitSystem> ("fiberSolution");
                            const DofMap & recovery_dof_map = fiberSoln.get_dof_map();
                            recovery_dof_map.dof_indices(elem, node_dof_indices);
                            ...
                            //if(elem -> subdomain_id() == 1){
                              //for (unsigned int qp=0; qp<qrule.n_points(); qp++){
                                double fx = fiberSoln.point_value(fibx_var, q_point[qp], elem);
                                double fy = fiberSoln.point_value(fiby_var, q_point[qp], elem);
                                double fz = fiberSoln.point_value(fibz_var, q_point[qp], elem);

                                double sx = fiberSoln.point_value(crossfx_var, q_point[qp], elem);
                                double sy = fiberSoln.point_value(crossfy_var, q_point[qp], elem);
                                double sz = fiberSoln.point_value(crossfz_var, q_point[qp], elem);

                                double nx = fiberSoln.point_value(sheetx_var, q_point[qp], elem);
                                double ny = fiberSoln.point_value(sheety_var, q_point[qp], elem);
                                double nz = fiberSoln.point_value(sheetz_var, q_point[qp], elem);
                                //totalErrorInSpaceV += JxW[qp]*((u_h - u_exact)*(u_h - u_exact));

                                //libMesh::out << "in elem qp fx is: " << fx << std::endl;

                                TensorValue<Number> fofp(fx * fx, fx * fy, fx * fz, fy * fx, fy * fy, fy * fz, fz * fx, fz * fy, fz * fz);
                                TensorValue<Number> sosp(sx * sx, sx * sy, sx * sz, sy * sx, sy * sy, sy * sz, sz * sx, sz * sy, sz * sz);
                                TensorValue<Number> nonp(nx * nx, nx * ny, nx * nz, ny * nx, ny * ny, ny * nz, nz * nx, nz * ny, nz * nz);

                                sigma_I_fibersV[indToUse] = ( sigma_f_i * fofp + sigma_s_i * sosp + sigma_n_i * nonp ) /chi;
                                sigma_E_fibersV[indToUse] = ( sigma_f_e * fofp + sigma_s_e * sosp + sigma_n_e * nonp ) / chi;
                                sigma_M_fibersV[indToUse] = ( sigma_f_M * fofp + sigma_s_M * sosp + sigma_n_M * nonp ) /chi;
                              //}
                            //}
                        }
                        ...
                        for (unsigned int i = 0; i != parabolic_ndofs; i++)
                        {
                            ...
                            for (unsigned int j = 0; j != parabolic_ndofs; j++)
                            {
                              ...
                              const Real xT = q_point[qp](0);
                              const Real yT = q_point[qp](1);
                              const Real zT = q_point[qp](2);
                              ...
                              if(fibrosisBool && fibrosisMethod.compare(0,14,"Conductivities") == 0){
                                ...
                                if(differentFibers){
                                    ...
                                    // stiffness matrix
                                    KVV(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_iloc/factorDiv) * dphi[j][qp]);
                                    KVVe(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_iloc/factorDiv) * dphi[j][qp]);
                                    KVeV(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_iloc/factorDiv) * dphi[j][qp]);
                                    KVeVe(i, j) += JxW[qp] * dphi[i][qp] * (((sigma_iloc + sigma_eloc)/factorDiv) * dphi[j][qp]);
                                    Ki(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_iloc/factorDiv) * dphi[j][qp]);
                                    Kmono(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_IMono/factorDiv) * dphi[j][qp]);
                                  
                                }
                                else{
                                  // stiffness matrix
                                  KVV(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_I_fibersV[indToUse]/factorDiv) * dphi[j][qp]);
                                  KVVe(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_I_fibersV[indToUse]/factorDiv) * dphi[j][qp]);
                                  KVeV(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_I_fibersV[indToUse]/factorDiv) * dphi[j][qp]);
                                  KVeVe(i, j) += JxW[qp] * dphi[i][qp] * (((sigma_I_fibersV[indToUse] + sigma_E_fibersV[indToUse])/factorDiv) * dphi[j][qp]);
                                  Ki(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_I_fibersV[indToUse]/factorDiv) * dphi[j][qp]);
                                  Kmono(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_M_fibersV[indToUse]/factorDiv) * dphi[j][qp]);
                                }
                              ...
                              if (it != bc_sidesets.end())
                              {
                                  ...
                                  for (unsigned int qp = 0; qp < qface.n_points(); qp++)
                                  {
                                      for (unsigned int i = 0; i != parabolic_ndofs; i++)
                                      {
                                          for (unsigned int j = 0; j != parabolic_ndofs; j++)
                                          {
                                              KVeVe(i, j) += JxW_face[qp] * penalty * phi_face[j][qp] * phi_face[i][qp];
                                          }
                                      }
                                  }
                              }
                      ...
                      for (unsigned int i = 0; i != elliptic_ndofs; i++)
                      {
                        for (unsigned int j = 0; j != elliptic_ndofs; j++)
                        {
                          ...
                          if(differentFibers){
                              ...
                              // stiffness matrix
                              KVVe(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_iloc + sigma_eloc) * dphi[j][qp]);
                            
                          }
                          else{
                            // stiffness matrix
                            KVVe(i, j) += JxW[qp] * dphi[i][qp] * ((sigma_I_fibersV[indToUse] + sigma_E_fibersV[indToUse]) * dphi[j][qp]);
                          }
                ...
                //libMesh::out << "HERE" << std::endl;
                //libMesh::out << "TRACKING BUG 4" << std::endl;
                ...
                system.get_matrix("Ki").add_matrix(Ki, parabolic_dof_indices, parabolic_dof_indices);
                system.get_matrix("Kmono").add_matrix(Kmono, parabolic_dof_indices, parabolic_dof_indices);
                if(fibrosisMethod.compare(0,14,"Conductivities") == 0 && fibrosisBool){
                  libMesh::LinearImplicitSystem &fibrosis_system = es.get_system < libMesh::LinearImplicitSystem > ("Fibrosis");
                  fibrosis_system.get_vector("Fibrosis_Check").add_vector(Fibrosis_CheckV, parabolic_dof_indices);
                }
            }
            //libMesh::out << "TRACKING BUG 5" << std::endl;
            system.matrix->close();
            system.get_matrix("M").close();
            system.get_vector("ML").close();
            system.get_vector("I_extra_stim").close();
            system.get_matrix("Ki").close();
            system.get_matrix("Kmono").close();
            if(fibrosisMethod.compare(0,14,"Conductivities") == 0 && fibrosisBool){
              libMesh::LinearImplicitSystem &fibrosis_system = es.get_system < libMesh::LinearImplicitSystem > ("Fibrosis");
                    ...
                    EquationType type)
{
    ...
    switch (time_integrator)
    {
        ...
        case TimeIntegrator::SEMI_IMPLICIT_HALF_STEP:
         {
            ...
            {
                //libMesh::out << "TRACKING BUG 0 IN" << std::endl;
                parabolic_system.get_vector("aux1").zero();
                ...
                //libMesh::out << "TRACKING BUG 1 IN" << std::endl;
                for (auto node : mesh.local_node_ptr_range())
                {
                    //libMesh::out << "TRACKING BUG 2 IN" << std::endl;
                    parabolic_dof_map.dof_indices(node, parabolic_dof_indices);
                    elliptic_dof_map.dof_indices(node, elliptic_dof_indices);
                    int elliptic_ndofs = elliptic_dof_indices.size();
                    int parabolic_ndofs = parabolic_dof_indices.size();
        ...
        {
            TransientLinearImplicitSystem &system = es.get_system < TransientLinearImplicitSystem > ("bidomain");
            TransientExplicitSystem &system2 = es.get_system < TransientExplicitSystem > ("Recovery");

            
            const MeshBase& mesh = es.get_mesh();
            const DofMap & elliptic_dof_map = system.get_dof_map();
            const DofMap & parabolic_dof_map = system2.get_dof_map();
            std::vector < dof_id_type > parabolic_dof_indices;
            std::vector < dof_id_type > elliptic_dof_indices;
            auto femSoluV = es.get_system("bidomain").variable_number("V");
        ...
        for(const auto & node : mesh.local_node_ptr_range()){
            ...
            if(dof_indices2.size() > 0){
                ...
                */
                ...
                double freact = 0.0;
                double I_ion = I_Na + I_K1 + I_to + I_Kur + I_Kr + I_Ks + I_CaL + I_pCa + I_NaK + I_NaCa + I_bNa + I_bCa;
                double Istim = 0.0;
                double Istim2 = 0.0;
...
void set_fiber_directions(libMesh::EquationSystems &es, const GetPot &data)
{
  ...
  fstream file2 (FiberInfoCSV, ios::in);
  if(file2.is_open())
  {
    while(getline(file2, line))
    {
    ...
    double fx, fy, fz, sx, sy, sz, nx, ny, nz;
    for(const auto & node : mesh.local_node_ptr_range()){
      recovery_dof_map.dof_indices(node, node_dof_indices);
